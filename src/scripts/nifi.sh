...
[ "$USE_ULTRAFAST" = "true" ] && UF_LDPRELOAD_OPTION="LD_PRELOAD=libultrax.so"
...
run() {
    ...
    run_nifi_cmd="$UF_LDPRELOAD_OPTION '${JAVA}' -cp '${BOOTSTRAP_CLASSPATH}' -Xms12m -Xmx24m ${BOOTSTRAP_DIR_PARAMS} ${BOOTSTRAP_DEBUG_PARAMS} ${BOOTSTRAP_JAVA_OPTS} org.apache.nifi.bootstrap.RunNiFi $@"
